...
def analyze_copy_move(doc: fitz.Document, doc_class: DocumentClass) -> List[ForensicFinding]:
    ...
    min_spatial_dist = cm_config.get("min_spatial_dist_px", 40)
    coherence_tol = cm_config.get("coherence_tolerance_px", 2)
    ...
    for page_idx, page in enumerate(doc):
        try:
            ...
            for label in unique_labels:
                ...
                # Text naturally repeats, but rigid copy-move produces near-identical
                # displacements (tolerance from config; spec: +-2px)
                mean_disp = cluster_disps.mean(axis=0)
                coherent = np.all(np.abs(cluster_disps - mean_disp) <= coherence_tol, axis=1)
                ...
                if np.sum(coherent) >= min_samples:
                    ...
                    # Union bbox in render pixels -> PDF points (canonical space)
                    overall_bbox_px = (
                        min(bbox1[0], bbox2[0]),
                        min(bbox1[1], bbox2[1]),
                        max(bbox1[2], bbox2[2]),
                    ...
                    overall_bbox = pixels_to_points(overall_bbox_px, RENDER_DPI)
